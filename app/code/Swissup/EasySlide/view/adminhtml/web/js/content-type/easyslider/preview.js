...
define([
  ...
  var Preview = /*#__PURE__*/function (_preview2) {
    ...
    function Preview(contentType, config, observableUpdater) {
      ...
      _this.displayingSliderPreview = _knockout.observable(false);
      _this.loading = _knockout.observable(false);
      _this.messages = {
        NOT_SELECTED: (0, _translate)("Slide isn't selected"),
    ...
    _proto.processSliderData = function processSliderData(data) {
      // Only load if something changed
      this.displayPreviewPlaceholder(data, "identifier");

      if (data.identifier && data.template.length !== 0) {
        this.processRequest(data, "identifier", "title");
      }
    }
    /**
     * @inheritdoc
    ...
    _proto.processRequest = function processRequest(data, identifierName, labelKey) {
      ...
      .done(function (response) {
        ...
        if (typeof response.data !== "object") {
          _this2.showSliderPreview(false);
    ...
    _proto.showSliderPreview = function showSliderPreview(isShow) {
      this.displayingSliderPreview(isShow);
    }
    /**
    ...
    _proto.processBackgroundImages = function processBackgroundImages(content) {
      ...
      var styleSlider = document.createElement("style");
      ...
      elements.forEach(function (element) {
        ...
        if (rules.length) {
          styleSlider.append(rules);
          element.classList.add(elementClass);
        }
      });

      if (elements.length && styleSlider.innerText.length) {
        document.body.append(styleSlider);
    ...
    _proto.processBreakpointStyles = function processBreakpointStyles(content) {
      var document = new DOMParser().parseFromString(content, "text/html");
      var styleSliders = document.querySelectorAll("style");
      var mediaStyleSlider = document.createElement("style");

      var viewports = _config.getConfig("viewports");

      styleSliders.forEach(function (styleSlider) {
        var cssRules = styleSlider.sheet.cssRules;
          ...
          if (mediaScope) {
            Array.from(rule.cssRules).forEach(function (mediaRule, index) {
              if (mediaRule.selectorText.indexOf(_styleRegistry.pbStyleAttribute) !== -1) {
                ...
                mediaStyleSlider.append(selector + " {" + mediaRule.style.cssText + "}");
              }
            });
          }
        });
      ...
      if (mediaStyleSlider.innerText.length) {
        document.body.append(mediaStyleSlider);
        content = document.head.innerHTML + document.body.innerHTML;
      }
