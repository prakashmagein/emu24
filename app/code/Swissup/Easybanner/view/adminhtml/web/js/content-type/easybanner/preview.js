...
define([
  ...
  var Preview = /*#__PURE__*/function (_preview2) {
    ...
    function Preview(contentType, config, observableUpdater) {
      ...
      _this.messages = {
        NOT_SELECTED: (0, _translate)("Banner isn't selected"),
    ...
    _proto.processBannerData = function processBannerData(data) {
      // Only load if something changed
      this.displayPreviewPlaceholder(data, "banner_id");

      if (data.banner_id && data.template.length !== 0) {
        this.processRequest(data, "banner_id", "title");
      }
    }
    /**
     * @inheritdoc
    ...
    _proto.displayPreviewPlaceholder = function displayPreviewPlaceholder(data, identifierName) {
      var bannerId = (0, _object.get)(data, identifierName); // Only load if something changed

      if (this.lastBannerId === bannerId && this.lastTemplate === data.template) {
      ...
      if (!bannerId || bannerId && bannerId.toString().length === 0 || data.template.length === 0) {
        this.showBannerPreview(false);
    ...
    _proto.processRequest = function processRequest(data, identifierName, labelKey) {
      ...
      this.loading(true); // Retrieve a state object representing the banner from the preview controller and process it on the stage

      _jquery.ajax(url, requestConfig) // The state object will contain the banner name and either html or a message why there isn't any.
      .done(function (response) {
        // Empty content means something bad happened in the controller that didn't trigger a 5xx
        if (typeof response.data !== "object") {
          _this2.showBannerPreview(false);

          _this2.placeholderText(_this2.messages.UNKNOWN_ERROR);

          return;
        } // Update the stage content type label with the real banner title if provided
        ...
        _this2.lastBannerId = parseInt(identifier.toString(), 10);
    ...
    /**
     * Toggle display of banner preview.  If showing banner preview, add hidden mode to PB preview.
     * @param {boolean} isShow
     */
    ;

    _proto.showBannerPreview = function showBannerPreview(isShow) {
    ...
    _proto.processBackgroundImages = function processBackgroundImages(content) {
      ...
      elements.forEach(function (element) {
        ...
        if (rules.length) {
          styleBanner.append(rules);
          element.classList.add(elementClass);
        }
      });

      if (elements.length && styleBanner.innerText.length) {
        document.body.append(styleBanner);
    ...
    _proto.processBreakpointStyles = function processBreakpointStyles(content) {
      var document = new DOMParser().parseFromString(content, "text/html");
      var styleBanners = document.querySelectorAll("style");
      var mediaStyleBanner = document.createElement("style");

      var viewports = _config.getConfig("viewports");

      styleBanners.forEach(function (styleBanner) {
        var cssRules = styleBanner.sheet.cssRules;
          ...
          if (mediaScope) {
            Array.from(rule.cssRules).forEach(function (mediaRule, index) {
              if (mediaRule.selectorText.indexOf(_styleRegistry.pbStyleAttribute) !== -1) {
                ...
                mediaStyleBanner.append(selector + " {" + mediaRule.style.cssText + "}");
              }
            });
          }
        });
      ...
      if (mediaStyleBanner.innerText.length) {
        document.body.append(mediaStyleBanner);
        content = document.head.innerHTML + document.body.innerHTML;
      }
