...
define([
  ...
  var Preview = /*#__PURE__*/function (_preview2) {
    ...
    function Preview(contentType, config, observableUpdater) {
      ...
      _this.messages = {
        NOT_SELECTED: (0, _translate)("Placeholder isn't selected"),
    ...
    _proto.processWidgetData = function processWidgetData(data) {
      ...
      this.displayPreviewPlaceholder(data, "placeholder_id");

      if (data.placeholder_id && data.template.length !== 0) {
        this.processRequest(data, "placeholder_id", "name");
      }
    }
    /**
     * @inheritdoc
    ...
    _proto.displayPreviewPlaceholder = function displayPreviewPlaceholder(data, identifierName) {
      var placeholderId = (0, _object.get)(data, identifierName); // Only load if something changed

      if (this.lastPlaceholderId === placeholderId && this.lastTemplate === data.template) {
        // The mass converter will have transformed the HTML property into a directive
        if (this.lastRenderedHtml) {
          this.data.main.html(this.lastRenderedHtml);
          this.showPlaceholderPreview(true);
          this.initializeWidgets(this.element);
        }
      } else {
        this.showPlaceholderPreview(false);
        this.placeholderText("");
      }

      if (!placeholderId || placeholderId && placeholderId.toString().length === 0 || data.template.length === 0) {
        this.showPlaceholderPreview(false);
    ...
    _proto.processRequest = function processRequest(data, identifierName, labelKey) {
      ...
      this.loading(true); // Retrieve a state object representing the placeholder from the preview controller and process it on the stage

      _jquery.ajax(url, requestConfig) // The state object will contain the placeholder name and either html or a message why there isn't any.
      .done(function (response) {
        // Empty content means something bad happened in the controller that didn't trigger a 5xx
        if (typeof response.data !== "object") {
          _this2.showPlaceholderPreview(false);

          _this2.placeholderText(_this2.messages.UNKNOWN_ERROR);

          return;
        } // Update the stage content type label with the real placeholder title if provided
        ...
        if (response.data.content) {
          _this2.showPlaceholderPreview(true);
        ...
        _this2.lastPlaceholderId = parseInt(identifier.toString(), 10);
    ...
    /**
     * Toggle display of placeholder preview.  If showing placeholder preview, add hidden mode to PB preview.
     * @param {boolean} isShow
     */
    ;

    _proto.showPlaceholderPreview = function showPlaceholderPreview(isShow) {
    ...
    _proto.processBackgroundImages = function processBackgroundImages(content) {
      ...
      elements.forEach(function (element) {
        ...
        if (rules.length) {
          stylePlaceholder.append(rules);
          element.classList.add(elementClass);
        }
      });

      if (elements.length && stylePlaceholder.innerText.length) {
        document.body.append(stylePlaceholder);
    ...
    _proto.processBreakpointStyles = function processBreakpointStyles(content) {
      var document = new DOMParser().parseFromString(content, "text/html");
      var stylePlaceholders = document.querySelectorAll("style");
      var mediaStylePlaceholder = document.createElement("style");

      var viewports = _config.getConfig("viewports");

      stylePlaceholders.forEach(function (stylePlaceholder) {
        var cssRules = stylePlaceholder.sheet.cssRules;
          ...
          if (mediaScope) {
            Array.from(rule.cssRules).forEach(function (mediaRule, index) {
              if (mediaRule.selectorText.indexOf(_styleRegistry.pbStyleAttribute) !== -1) {
                ...
                mediaStylePlaceholder.append(selector + " {" + mediaRule.style.cssText + "}");
              }
            });
          }
        });
      ...
      if (mediaStylePlaceholder.innerText.length) {
        document.body.append(mediaStylePlaceholder);
        content = document.head.innerHTML + document.body.innerHTML;
      }
